# 当点击“查看详情后”跳转到EQ Manage Page时，设置了自动筛选（Pending&Reviewing按照不同时间的筛选条件）
# 一定要测试跳转和筛选是否成功
# 且应当进一步考虑Dataset中的数据结构，例如Date of Creation和Date of Closed应当分开
...
from message import DASHBOARD_MESSAGES

# 获取语言与对应字典
lang = st.session_state.get("language", "zh-CN")
T = DASHBOARD_MESSAGES[lang]
...
def count_status(data):
    ...
    for item in data:
        ...
        if status == "Reviewing":
            ...
            closed += 1
    return {"Reviewing": reviewing, "Pending": pending, "Closed": closed}
        ...
        if status in ["Reviewing", "Pending"]:
            ...
            if creation_date_str:
                try:
                    ...
                    print(f"日期解析错误: {e}, 日期字符串: {creation_date_str}")
    ...
    with col2:
        ...
        with st.container():
            st.markdown(
                ...
                f"<h3 style='color:#b91c1c; font-size:1.5rem; margin:0;text-align:center'>超过7天</h3>"
                f"<p style='color:#b91c1c; font-size:2rem; font-weight:bold; margin:8px 0 0 0;text-align:center'>{over_7_days} EQs</p>"
                f"</div>",
                unsafe_allow_html=True
            )
            st.button(T["viewDetails"], key="over_7_days_btn", on_click=navigate_to_eq_manage, args=("over7",), use_container_width=True)

        with st.container():
            st.markdown(
                f"<div style='background-color:#fef3c7; padding:10px; border-radius:5px;text-align:center;'>"
                f"<h3 style='color:#b45309; font-size:1.5rem; margin:0; text-align:center'>2至7天内</h3>"
                f"<p style='color:#b45309; font-size:2rem; font-weight:bold; margin:8px 0 0 0;text-align:center'>{between_2_7_days} EQs</p>"
                f"</div>",
                unsafe_allow_html=True
            )
            st.button(T["viewDetails"], key="between_2_7_days_btn", on_click=navigate_to_eq_manage, args=("between2_7",), use_container_width=True)

        with st.container():
            st.markdown(
                f"<div style='background-color:#d1fae5; padding:10px; border-radius:5px;text-align:center'>"
                f"<h3 style='color:#047857; font-size:1.5rem; margin:0;text-align:center'>少于2天</h3>"
                f"<p style='color:#047857; font-size:2rem; font-weight:bold; margin:8px 0 0 0;text-align:center'>{under_2_days} EQs</p>"
                f"</div>",
                unsafe_allow_html=True
            )
            st.button(T["viewDetails"], key="under_2_days_btn", on_click=navigate_to_eq_manage, args=("under2",), use_container_width=True)

if st.session_state.navigate_to == "eq_manage":
    days_filter = st.session_state.filter_days
    st.session_state.navigate_to = None
    st.success(f"导航到EQ管理页面，筛选条件: {days_filter}")
